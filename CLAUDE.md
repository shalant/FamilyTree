...
- **PDF / Document** tab calls `ExtractFromDocumentAsync(bytes, fileName, startPage, endPage)`, which extracts text first (iText7 for `.pdf`, raw UTF-8 decode for `.txt`) then hands off to the same `ExtractFromTextAsync` path. `.doc`/`.docx` aren't supported yet — throws `NotSupportedException` telling the user to paste text instead.
- **GEDCOM** and **Excel/CSV** tabs are still UI stubs (`ProceedAsync` just shows a toast) — no extraction wired up.

**Page-range selection:** on upload, `ImportFormPanel` reads the PDF page count via `IImportService.GetPdfPageCount(bytes)` and offers optional "extract pages X–Y" inputs. `ExtractTextFromPdf` clamps the range (1-based, inclusive, out-of-range can't throw). Lets a huge document be imported a branch/chapter at a time — smaller, cheaper Claude calls. Each range is its own self-contained extraction and its own rollback batch; it splits the *document*, not the *family graph*, so cross-page relationships need the matching step below.

**Match-to-existing (entity resolution):** after extraction, `AnnotateMatchesAsync` loads the family's existing people and runs `ImportMatchService.FindCandidates` (pure, no DB) per imported person — fuzzy first+last name (diacritic-stripped, short edit-distance so Sara/Sarah match), alias and maiden names as extra keys, weighted with birth-year proximity; a hard year conflict (both known, far apart) excludes a pair even if names match. It's **suggestion-only**: the preview shows "↔ Possible match / Link / Create new instead" and sets `ImportPersonDto.MatchedExistingId`; default is always Create new. On `CommitAsync`, a linked person's temp id maps to the *existing* Guid (no duplicate created), so the import's relationships graft onto the real tree instead of forming an island. Existing relationships touching a linked person are preloaded into the `seen` set first to respect the unique `(PersonAId, PersonBId, Type)` index.

Approval and rollback: `CommitAsync` only persists people the user left checked in the preview, tags every created `Person` *and every created `Relationship`* with the `ImportBatchId`. `RollbackBatchAsync` soft-deletes (`DeletedAt`) every person in the batch plus every relationship carrying that batch tag — this covers "bridging" links to pre-existing people (one endpoint outside the batch) that the old both-endpoints-in-batch sweep would miss; a legacy fallback still handles pre-tag batches. The pre-existing matched person is never touched, so linking is fully reversible. Nothing is hard-deleted — an import is always reversible from the batch list.
