...
- **Known gap, not yet fixed**: which real bug (if any) was causing "the navbar looked broken" in production couldn't be confirmed via live mobile-viewport testing this session — the available browser automation couldn't actually shrink its render viewport to trigger the `max-width:768px` breakpoint (`window.innerWidth` stayed desktop-sized regardless of requested window size). The dead tap-target this same work fixed (`CustomAppBar`'s mobile identity block previously had `pointer-events:none` with no real handler behind it) was the leading suspect and should now be resolved as a side effect, but this needs confirming on a real device/browser DevTools before being called closed.
