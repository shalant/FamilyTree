...
Most responsive behavior (AppBar swapping search for a hamburger + centered user identity, drawers going full-width, etc.) is pure CSS via a single `@media (max-width: 768px)` block in `app.css` — no Blazor/JS involvement. A few pieces need more than CSS:

- **Canvas touch/pointer gestures** (2026-08-08): `canvas-interaction.js` is Pointer-Events-based, not mouse-only — one code path handles mouse, touch, and pen. `setPointerCapture` on `pointerdown` means drag-continuation needs no `window`-level fallback listeners; a second simultaneous pointer starts pinch-to-zoom (tracked via `_activePointers`, reusing the existing `_applyZoomAt` zoom-toward-a-point math against the pinch midpoint) instead of panning. `.ft-viewport` keeps `touch-action: none` — that's intentional, it's exactly what the Pointer Event handlers replace; there's no native-scroll fallback because the canvas is meant to pan/zoom internally, never the page (see `TreeLayout.razor`'s `overflow: hidden` lock, applied at every viewport width).
...
- **Mobile control panel replaces the floating toolbar's inline expand + the hero overlay** (2026-08-08): on mobile, `CustomToolbar` only ever renders its collapsed mini pill — the inline `.ft-toolbar-full` expand (and its old full-width-footer docking CSS) is desktop-only now. When `ForceCollapsed`, tapping the mini pill's expand arrow calls `TreeContextService.ToggleMobilePanel()` instead of setting local `_collapsed = false`. `CustomAppBar`'s mobile identity block does the same on tap. Both open the same `MobileControlPanel` (focus-person info + Zoom In/Out/Center/Reset), rendered once by `Home.razor` and gated entirely on `TreeContext.MobilePanelOpen` — no page-owned boolean, matching the pre-existing `TreeContext.RequestCenterTree()`/`OnCenterTreeRequested` cross-component pattern (now extended with `RequestZoomIn`/`RequestZoomOut`/`RequestResetView` counterparts, subscribed to by `Home.razor`). `HeroOverlayComponent` stays desktop-only (`display:none` on mobile, unchanged) — `MobileControlPanel` is its mobile replacement, not an addition alongside it. The `#ft-toolbar` wrapper's mobile CSS forces `top`/`right` to `auto !important` alongside `bottom`/`left`, so a stray inline position from a previously-restored desktop drag can't leak through the async `OnViewportWidthChanged` round trip.
- **Testing mobile CSS when the automation environment can't shrink its own viewport** (2026-09-10): confirmed again this session — `resize_window`-style browser-automation calls report success but `window.innerWidth` stays pinned at the real desktop width regardless of the requested size, and neither toggling native DevTools' device toolbar nor polling `setTimeout`/`requestAnimationFrame` for fine-grained timing is reliable in this environment (a backgrounded/non-focused automation tab gets its timers throttled or fully suspended by Chrome). The workaround: temporarily widen `@media (max-width: 768px)` to something absurd like `@media (max-width: 1600px)` in `app.css`, exercise the real desktop-width render against the now-active "mobile" rules, then revert before committing — this drives the actual rendering engine rather than faking a screenshot, so it's trustworthy for CSS-only behavior. It does *not* cover the C#-side `_isNarrowViewport`/`ForceCollapsed` plumbing (that's driven by `OnViewportWidthChanged` receiving a real `window.innerWidth`, unaffected by the CSS breakpoint value), nor real-device touch-input quirks — see the next entry for a bug this technique's desktop-click testing missed entirely.
- **Mobile nav drawer real-device bugs (2026-09-10/11)**: a round of real-phone testing (the technique above, then confirmed live by the user on an actual Android device) found several concrete bugs the previous "known gap" above left unconfirmed:
  - `.ft-drawer-close` (27×26px) and `.ft-drawer-item` rows (38px tall) were both under the 44×44 touch-target minimum despite `app.css` documenting that standard elsewhere (`#ft-toolbar` icon buttons) — fixed with explicit `min-width`/`min-height` rules.
  - The drawer panel's `::before` radial-gradient "glass sheen" (fading from 0.32 to 0.04 alpha, top-left to bottom-right) read as a visible two-tone banding seam once the drawer held more than a couple of rows — this is the *same class* of bug a code comment already documented fixing once (a flat-tail banding ring), just recurring in a new shape over a taller panel. Removed the gradient entirely rather than re-tuning it a third time; bumped the flat base opacity slightly (0.58→0.72 light, 0.52→0.66 dark) to keep it legible without the gradient's help.
  - The panel was capped at a flat `max-height: 55vh`, which read as a large dead backdrop area under a short list on a real phone (vh doesn't account for a mobile browser's collapsing/expanding chrome the way `dvh` does). Changed to `calc(100dvh - 76px)` (with a plain-`vh` fallback line before it for browsers without `dvh` support) so it fills nearly the whole screen and only truly-long content ever needs the `overflow-y: auto` that was already there.
  - The drawer had an entrance animation (`ft-nav-drop`, a bounce-settle) but no exit — closing just deleted the DOM node instantly. Added a real two-phase close in `CustomAppBar` (apply a `--closing` class, `await Task.Delay(250)`, then unmount) — but the first version of the closing CSS played `ft-nav-drop` in *reverse* rather than using its own keyframes, and that keyframe's opacity holds flat at 1 for its first 60% (the bounce-settle part), so reversed it barely moved for the first half of the close and read as no animation at all. Replaced with a dedicated `ft-nav-close` keyframe (`opacity`+`translateY`+`scaleY` ramping together across the full duration) instead of reusing the entrance one in reverse.
  - Both `--closing` CSS classes now also carry `pointer-events: none` as a deliberate safety net: `CustomAppBar` is rendered by the layout, so `_drawerOpen`/`_drawerClosing` persist across page navigation within the same layout — if the delayed unmount above ever fails to complete (an exception mid-`await`, a disposed-component edge case), the backdrop and panel would otherwise stay mounted indefinitely at `opacity: 0` while still `position: fixed; inset: 0` at a very high z-index, silently eating every click on the page underneath. `pointer-events: none` turns that failure mode from "the whole app looks dead until reload" into "an invisible, harmless leftover."
  - **Real regression, not just untested code**: the AppBar identity block's caret (tap to open `MobileControlPanel`) is gated by `CanRevealMobileToolbar`, which checks `NavigationManager.ToBaseRelativePath(Nav.Uri)` against an empty string to confirm the current route is "/". `ToBaseRelativePath` keeps the query string attached, so any `?focus=<id>` URL (a normal, common state — a shared tree link, or wherever focus gets persisted) evaluated as non-empty and wrongly read as "not home," hiding the caret and no-op'ing the tap. Found live by the user comparing two open browser sessions on the same account that rendered differently — one had a plain `/` URL, the other a `?focus=` one. Fixed by splitting off the query string/fragment before comparing (`Split('?', '#')[0]`); regression test (`CustomAppBarTests.TappingIdentityBlock_TogglesMobilePanel_WhenUrlHasFocusQueryString`) uses the exact URL that surfaced it, and was verified to actually fail against the pre-fix code before being confirmed passing against the fix — not just written and assumed correct.
- **Page-header toolbars don't fit a phone row** (2026-09-10): `People.razor`'s header (title + chip on one side, search field + status filter + add-person button on the other, `flex-wrap: nowrap`) needs ~789px of width for its right-hand group alone — far more than a phone screen, and would have clipped or forced page-wide horizontal scroll. `Stories.razor`'s equivalent header is more marginal (~356px needed) but close enough to an iPhone SE's available width to apply the same fix for consistency. Added a shared `.ft-page-toolbar`/`.ft-people-actions` wrap treatment in the mobile media query rather than a one-off fix per page. Separately, `People.razor`'s 5-column table (Name/Born/Birthplace/Died/Age) had no mobile treatment at all — contained it to scroll horizontally within its own box (`.people-table .mud-table-container { overflow-x: auto }`) instead of letting it force the whole page to scroll.
- **`PersonForm.razor` ignored phone width entirely** (2026-09-10): every field pair in the Identity card (`First name`/`Last name`, `Date of birth`/`Place of birth`, etc.) used a fixed `MudItem xs="6"` (or `5`/`7`) — MudBlazor's `xs` breakpoint is the *smallest* one and applies at every width unless a larger breakpoint prop overrides it, so these stayed half-width even on the narrowest phones with no `sm`/`md` override to stack them. This is the same shared component used by both Add and Edit Person — the exact flow that caused real user (Doug's dad) frustration in an earlier session. Changed all 10 fields to `xs="12" sm="<original value>"` — full-width below the `sm` (~600px) breakpoint, unchanged above it.
...
| `HeroOverlayComponent` | Floating info card (focus person stats); draggable; desktop only — mobile uses `MobileControlPanel` instead |
| `CustomToolbar` | Floating toolbar (zoom, center, reset); draggable on desktop. On mobile (`ForceCollapsed`) it's a fixed mini pill whose expand arrow opens `MobileControlPanel` instead of expanding inline |
| `MobileControlPanel` | Mobile-only combined panel (focus person info + zoom/center/reset); opened via the AppBar identity tap or the toolbar's mini pill, both through `TreeContextService`'s `MobilePanelOpen`/`ToggleMobilePanel`/`CloseMobilePanel` |
...
- **Comprehensive test suites**: `FamilyTreeLayoutEngineTests` (regression suite with before/after pairs for classification transitions), `RelationshipServiceTests`, `AuthServiceTreeLinkingTests`, and integration tests hitting an EF Core InMemory database (a real EF pipeline and real `UserManager<AppUser>`, not mocked services — see `TestInfrastructure.cs`; not literally SQL Server, unlike the E2E suite below)
...
- **Integration testing**: stories and invites hit a real EF Core pipeline; auth flows tested end-to-end; layout engine tested against known genealogies (cross-root couples, orphaned siblings, multiple marriages)
...
Tests protecting a specific bug that shipped and was found live (not just ordinary unit-test coverage) carry `[Trait("Category", "Regression")]` — xUnit's built-in trait mechanism, no custom attribute. `dotnet test --filter "Category=Regression"` runs just that set; `ci.yml` runs it as a second, informational step after the main suite so a broken regression is easy to spot separately in CI logs. When you fix a bug that was found via manual/live testing rather than caught by an existing test, add the regression test *and* tag it — that's what closes the gap for next time. Before writing one, verify it actually distinguishes buggy from fixed behavior: temporarily revert the fix, confirm the new test fails, then restore the fix and confirm it passes — a test that passes identically either way (see the two gotchas below) gives false confidence.

Two gotchas found writing these (2026-09-11), both about a test appearing to pass regardless of the underlying bug:
- **bUnit silently strips raw NUL characters during HTML parsing** (AngleSharp's tokenizer). A DOM-text assertion for a bug whose signature was a `\0` character (`PersonForm`'s "tofu glyph" avatar-initials bug) passes identically whether the bug is present or fixed — the DOM never shows it either way. Fix: reflect on the component's private computed property directly (`typeof(PersonForm).GetProperty("Initials", NonPublic | Instance)`) rather than asserting on rendered text, when the bug's signature is a control character.
- **A test that re-locates an element by content (e.g. `FindAll("tr").First(r => r.TextContent.Contains(name))`) can't reproduce a position-shift bug** — it just re-finds "Bob" wherever he ends up after a re-render, silently working around the exact bug it's meant to catch (`DeletedTab`'s restore-list-shift regression). Fix: interact by fixed index/position (`FindAll("tbody tr")[1]`), matching how a real user clicks a physical screen location without re-reading labels each time.

### Real Browser E2E Tests (`FamilyTree.E2E.Tests`)
bUnit diffs a virtual DOM — it cannot see real CSS layout facts (touch target size, computed `max-height`, animation timing) or real browser behavior (native form POSTs, cookie auth, a real viewport). `tests/FamilyTree.E2E.Tests` closes that gap: it launches the actual built `FamilyTree.Web.dll` as a real external OS process (not `WebApplicationFactory`/`TestServer` — `Program.cs` has no `public partial class Program` marker, and forcing `WebApplicationFactory` to bind a real Kestrel port instead of its default in-memory `TestServer` is a known-fragile trick) against a **real, disposable SQL Server database** (not SQLite/InMemory — `Person`/`Relationship` use SQL Server's `RowVersion` for optimistic concurrency, which SQLite has no equivalent for), and drives it with Playwright's headless Chromium.

- **`E2eAppFixture`** (`ICollectionFixture`, boots once per test run): resets the test DB (`EnsureDeletedAsync`, then lets the app's own unconditional `MigrateAsync()` on startup recreate schema — the same path production uses), launches the app process with env-var overrides, polls until it responds, then hands out a shared `IBrowser`. Defensively overrides `Email__SmtpHost`, `SuperUser__Email`, `Google__Client*`, and `AzureStorage__ConnectionString` to empty strings — **without this, a developer's real Gmail SMTP credentials, real Azure Storage account, and real SuperUser email (all present in local user secrets per this file's own setup docs) would flow straight into the E2E run**, since `ASPNETCORE_ENVIRONMENT=Development` (required — see below) loads user secrets. If the DB is unreachable, tests **fail loudly** with a clear message (`E2eTestBase`'s constructor) rather than silently skip — xUnit v2 has no built-in dynamic skip, and a silently-passing "skip" would give false confidence about coverage that never ran. `E2E_ConnectionString` env var overrides the default local-SQLEXPRESS-with-a-distinct-DB-name fallback.
- **Never uses `DevAuth`**: `DevAuth:Enabled=true` replaces the *entire* cookie/Identity authentication pipeline Program.cs registers (see Auth & Security below) — real registration and real login become structurally impossible to exercise under it. `AuthFlowHelper` drives real registration/login through the actual UI instead. `TestDataSeeder` inserts arrange-phase data (a `Family`, a `UserInvite`, a password-hashed `AppUser` via `PasswordHasher<AppUser>`) directly through `AppDbContext`, sidestepping the need for an authenticated admin browser session just to seed a valid invite link.
- **A seeded test user needs a `UserFamily` row, not just a `Person`**: `PersonService.GetAllAsync()` deliberately shows a regular (non-super-user) account **nothing** if their `FamilyId` claim is missing ("must see NOTHING rather than being accidentally treated the same as a super-user") — that claim only comes from a real `UserFamily` row via `AppUserClaimsPrincipalFactory`. A user seeded without one logs in fine but sees a permanently empty tree regardless of what `Person` rows exist. `TestDataSeeder.CreateLinkedUserAsync` takes `familyId` as a required (not optional) parameter for exactly this reason.
- **`ASPNETCORE_ENVIRONMENT=Development` is required** for the app to boot at all in this harness — `app.UseHttpsRedirection()` runs unconditionally regardless of environment (so the harness must serve HTTPS, using this machine's trusted `dotnet dev-certs` cert), and Blazor Server prerenders the component tree statically first, then the interactive SignalR circuit reconnects and **re-initializes the component as a second, distinct instance** — confirmed via the app's own request log showing `ValidateTokenAsync` invoked twice for one navigation. Typing into a form before that second instance takes over gets silently discarded when the interactive render replaces the prerendered DOM. `page.WaitForLoadStateAsync(LoadState.NetworkIdle)` right after every `GotoAsync` is the fix (the circuit's WebSocket upgrade is itself an HTTP request Playwright tracks) — but `NetworkIdle` does **not** cover async work the circuit does *after* that, over the WebSocket (ordinary SignalR traffic isn't HTTP): a login form's hidden mirror inputs (`LoginOverlay.razor`'s real `<form method="post">` submits `<input type="hidden" name="email" value="@_email">`, kept in sync by a separate `Immediate="true"` round-trip, not the visible field's own value) can still be stale at submit time if you don't also wait for their actual value (`Expect(locator).ToHaveValueAsync(...)`) before clicking submit.
- **`/auth/do-login`'s rate limiter (5 requests per 15 min per IP) is real and applies to the E2E suite too** — it's a deliberate production security feature, not something to weaken for tests. Every test method independently logging in adds up fast; `MobileViewportTests` logs in **once** (a static, lazily-initialized, lock-guarded `Task`) and reuses Playwright's `StorageStateAsync()` cookie state across its own separate browser contexts per test, rather than a fresh real login per test method. Running the whole suite more than once or twice within the same 15-minute window (e.g. iterating locally) can still trip it — if a test fails with `?loginError=toomany`, that's the cause, not a real regression.
- **CI** (`ci.yml`'s `e2e-test` job): a disposable `mcr.microsoft.com/mssql/server` service container (throwaway SA password, destroyed with the job — never holds real data) plus `Microsoft.Playwright`'s own generated `playwright.ps1` install script (guarantees the installed browser version always matches the referenced package version, no separate CLI tool to keep in sync). Runs as a separate job from the main `build-and-test`, which explicitly excludes this project — it fails loudly, not skips, without the container present.
