...
- **FIXED (2026-07-08)** — A signed-in user's own linked Person being soft-deleted used to silently drop them onto an arbitrary stranger's tree (`_currentUserIsUnlinked`/the recovery-modal trigger only checked `PersonId.HasValue`, not whether it still resolved). Only unit-tested (`LinkedPersonResolverTests`) — never clicked through live, so please run: (1) sign in as a non-superuser Member, note your own linked person's name; (2) as Doug (superuser), delete that Member's linked Person via Admin; (3) reload the tree as that Member — confirm you land on the "Choose your starting point" empty state, **not** someone else's tree, and the "Complete your profile" recovery modal opens automatically offering to relink; (4) confirm picking a person (or creating a new one) in the modal works normally and the tree then centers on the newly-linked person.
